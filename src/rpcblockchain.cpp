...
Value gettxoutsbyaddress(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(Value& input, inputs) {
        ...
        pcoinsByAddress->GetCoinsByAddress(script, coinsByAddress);

        if (nMinDepth == 0)
            mempool.GetCoinsByAddress(script, coinsByAddress);

        BOOST_FOREACH(const COutPoint &outpoint, coinsByAddress.setCoins)
        {
            CCoins coins;
            if (nMinDepth == 0)
            {
