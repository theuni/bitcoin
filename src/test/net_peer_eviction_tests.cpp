...
{
    Shuffle(candidates.begin(), candidates.end(), random_context);
    const std::optional<NodeId> evicted_node_id = SelectIncomingNodeToEvict(std::move(candidates));
    if (!evicted_node_id) {
        return false;
    }
...
BOOST_AUTO_TEST_CASE(peer_eviction_test)
{
    ...
    for (int number_of_nodes = 0; number_of_nodes < 200; ++number_of_nodes) {
        ...
        if (number_of_nodes >= 29) {
            BOOST_CHECK(SelectIncomingNodeToEvict(GetRandomNodeEvictionCandidates(number_of_nodes, random_context)));
        }
