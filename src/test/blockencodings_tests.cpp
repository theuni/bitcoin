...
BOOST_AUTO_TEST_CASE(SimpleRoundTripTest)
{
    ...
    BOOST_CHECK_EQUAL(pool.iters_by_txid.find(block.vtx[2]->GetHash())->second->first.GetSharedTx().use_count(), SHARED_TX_OFFSET + 0);
    ...
    // Do a simple ShortTxIDs RT
    {
        ...
        BOOST_CHECK_EQUAL(pool.iters_by_txid.find(block.vtx[2]->GetHash())->second->first.GetSharedTx().use_count(), SHARED_TX_OFFSET + 1);
...
BOOST_AUTO_TEST_CASE(NonCoinbasePreforwardRTTest)
{
    ...
    // Test with pre-forwarding tx 1, but not coinbase
    {
        ...
        BOOST_CHECK_EQUAL(pool.iters_by_txid.find(block.vtx[2]->GetHash())->second->first.GetSharedTx().use_count(), SHARED_TX_OFFSET + 1); // +1 because of partialBlock
        ...
        BOOST_CHECK_EQUAL(pool.iters_by_txid.find(block.vtx[2]->GetHash())->second->first.GetSharedTx().use_count(), SHARED_TX_OFFSET + 2); // +2 because of partialBlock and block2
        bool mutated;
        BOOST_CHECK(block.hashMerkleRoot != BlockMerkleRoot(block2, &mutated));
        ...
        BOOST_CHECK_EQUAL(pool.iters_by_txid.find(block.vtx[2]->GetHash())->second->first.GetSharedTx().use_count(), SHARED_TX_OFFSET + 3); // +2 because of partialBlock and block2 and block3

        txhash = block.vtx[2]->GetHash();
        block.vtx.clear();
        block2.vtx.clear();
        block3.vtx.clear();
        BOOST_CHECK_EQUAL(pool.iters_by_txid.find(txhash)->second->first.GetSharedTx().use_count(), SHARED_TX_OFFSET + 1 - 1); // + 1 because of partialBlock; -1 because of block.
    }
    BOOST_CHECK_EQUAL(pool.iters_by_txid.find(txhash)->second->first.GetSharedTx().use_count(), SHARED_TX_OFFSET - 1); // -1 because of block
}
...
BOOST_AUTO_TEST_CASE(SufficientPreforwardRTTest)
{
    ...
    BOOST_CHECK_EQUAL(pool.iters_by_txid.find(block.vtx[1]->GetHash())->second->first.GetSharedTx().use_count(), SHARED_TX_OFFSET + 0);
    ...
    // Test with pre-forwarding coinbase + tx 2 with tx 1 in mempool
    {
        ...
        BOOST_CHECK_EQUAL(pool.iters_by_txid.find(block.vtx[1]->GetHash())->second->first.GetSharedTx().use_count(), SHARED_TX_OFFSET + 1);
