...
FUZZ_TARGET(node_eviction)
{
    ...
    // indeterminate state after the SelectIncomingNodeToEvict(&&) call.
    const std::vector<NodeEvictionCandidate> eviction_candidates_copy = eviction_candidates;
    const std::optional<NodeId> node_to_evict = SelectIncomingNodeToEvict(std::move(eviction_candidates));
    if (node_to_evict) {
