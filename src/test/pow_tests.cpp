...
{
    const auto consensus = CreateChainParams(CBaseChainParams::MAIN, m_node.args->GetOverrides())->GetConsensus();
    uint256 hash;
    unsigned int nBits;
...
{
    const auto chainParams = CreateChainParams(chainName, args.GetOverrides());
