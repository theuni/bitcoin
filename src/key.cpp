...
bool CKey::Derive(CKey& keyChild, CChainCode &ccChild, unsigned int nChild, const CChainCode& cc) const {
    assert(IsValid());
    assert(IsCompressed());
...
void CExtKey::Encode(unsigned char code[74]) const {
    ...
    memcpy(code+9, chaincode.data, chaincode.size);
    code[41] = 0;
    assert(key.size() == 32);
