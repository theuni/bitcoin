...
KeyPair::KeyPair(const CKey& key, const uint256* merkle_root)
{
    ...
    if (!secp256k1_keypair_create(secp256k1_context_sign, (secp256k1_keypair*)keydata->data(), UCharCast(key.data()))) return;
    if (merkle_root) {
        secp256k1_xonly_pubkey pubkey;
        if (!secp256k1_keypair_xonly_pub(secp256k1_context_sign, &pubkey, nullptr, (secp256k1_keypair*)keydata->data())) return;
        unsigned char pubkey_bytes[32];
        if (!secp256k1_xonly_pubkey_serialize(secp256k1_context_sign, pubkey_bytes, &pubkey)) return;
        uint256 tweak = XOnlyPubKey(pubkey_bytes).ComputeTapTweakHash(merkle_root->IsNull() ? nullptr : merkle_root);
        if (!secp256k1_keypair_xonly_tweak_add(secp256k1_context_static, (secp256k1_keypair*)keydata->data(), tweak.data())) return;
    }
