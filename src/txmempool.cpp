                                      ...
                                      const std::set<uint256>& setExclude, std::set<uint256>& descendants_to_remove)
{
    ...
    while (!stageEntries.empty()) {
        ...
        for (const CTxMemPoolEntry& childEntry : children) {
            cacheMap::iterator cacheIt = cachedDescendants.find(mapTxTxidIndex.iterator_to(childEntry));
    ...
    for (const CTxMemPoolEntry& descendant : descendants) {
        if (!setExclude.count(descendant.GetTx().GetHash())) {
            ...
            modifyCount++;
            cachedDescendants[updateIt].insert(mapTxTxidIndex.iterator_to(descendant));
            // Update ancestor state for each descendant
            mapTxTxidIndex.modify(mapTxTxidIndex.iterator_to(descendant), [=](CTxMemPoolEntry& e) {
    ...
    mapTxTxidIndex.modify(updateIt, [=](CTxMemPoolEntry& e) { e.UpdateDescendantState(modifySize, modifyFee, modifyCount); });
}
...
void CTxMemPool::UpdateTransactionsFromBlock(const std::vector<uint256>& vHashesToUpdate)
{
    ...
    for (const uint256 &hash : reverse_iterate(vHashesToUpdate)) {
        ...
        txiter it = mapTxTxidIndex.find(hash);
        if (it == mapTxTxidIndex.end()) {
            continue;
        }
        ...
        // we cache the in-mempool children to avoid duplicate updates
        {
            ...
            for (; iter != mapNextTx.end() && iter->first->hash == hash; ++iter) {
                ...
                txiter childIter = mapTxTxidIndex.find(childHash);
                assert(childIter != mapTxTxidIndex.end());
    ...
    const Limits& limits) const
{
    ...
    while (!staged_ancestors.empty()) {
        ...
        txiter stageit = mapTxTxidIndex.iterator_to(stage);
        ...
        for (const CTxMemPoolEntry& parent : parents) {
            txiter parent_it = mapTxTxidIndex.iterator_to(parent);
    ...
    bool fSearchForParents /* = true */) const
{
    ...
    if (fSearchForParents) {
        ...
        txiter it = mapTxTxidIndex.iterator_to(entry);
...
void CTxMemPool::UpdateAncestorsOf(bool add, txiter it, setEntries &setAncestors)
{
    ...
    for (const CTxMemPoolEntry& parent : parents) {
        UpdateChild(mapTxTxidIndex.iterator_to(parent), it, add);
    }
    const int32_t updateCount = (add ? 1 : -1);
    const int32_t updateSize{updateCount * it->GetTxSize()};
    const CAmount updateFee = updateCount * it->GetModifiedFee();
    for (txiter ancestorIt : setAncestors) {
        mapTxTxidIndex.modify(ancestorIt, [=](CTxMemPoolEntry& e) { e.UpdateDescendantState(updateSize, updateFee, updateCount); });
    }
}
...
void CTxMemPool::UpdateEntryForAncestors(txiter it, const setEntries &setAncestors)
{
    ...
    mapTxTxidIndex.modify(it, [=](CTxMemPoolEntry& e){ e.UpdateAncestorState(updateSize, updateFee, updateCount, updateSigOpsCost); });
}
...
void CTxMemPool::UpdateChildrenForRemoval(txiter it)
{
    ...
    for (const CTxMemPoolEntry& updateIt : children) {
        UpdateParent(mapTxTxidIndex.iterator_to(updateIt), it, false);
    }
}
    ...
    if (updateDescendants) {
        ...
        for (txiter removeIt : entriesToRemove) {
            ...
            for (txiter dit : setDescendants) {
                mapTxTxidIndex.modify(dit, [=](CTxMemPoolEntry& e){ e.UpdateAncestorState(modifySize, modifyFee, -1, modifySigOps); });
            }
        }
    }
    for (txiter removeIt : entriesToRemove) {
...
void CTxMemPool::addUnchecked(const CTxMemPoolEntry &entry, setEntries &setAncestors)
{
    ...
    indexed_transaction_set::iterator newit = mapTxTxidIndex.emplace(CTxMemPoolEntry::ExplicitCopy, entry).first;
    ...
    if (delta) {
        mapTxTxidIndex.modify(newit, [&delta](CTxMemPoolEntry& e) { e.UpdateModifiedFee(delta); });
    }
...
void CTxMemPool::CalculateDescendants(txiter entryit, setEntries& setDescendants) const
{
    ...
    while (!stage.empty()) {
        ...
        for (const CTxMemPoolEntry& child : children) {
            txiter childiter = mapTxTxidIndex.iterator_to(child);
...
void CTxMemPool::removeRecursive(const CTransaction &origTx, MemPoolRemovalReason reason)
{
    ...
    AssertLockHeld(cs);
        ...
        txiter origit = mapTxTxidIndex.find(origTx.GetHash());
        if (origit != mapTxTxidIndex.end()) {
            ...
            for (unsigned int i = 0; i < origTx.vout.size(); i++) {
                ...
                if (it == mapNextTx.end())
                    continue;
                txiter nextit = mapTxTxidIndex.find(it->second->GetHash());
                assert(nextit != mapTxTxidIndex.end());
...
void CTxMemPool::removeForReorg(CChain& chain, std::function<bool(txiter)> check_final_and_mature)
{
    ...
    setEntries txToRemove;
    for (indexed_transaction_set::const_iterator it = mapTxTxidIndex.begin(); it != mapTxTxidIndex.end(); it++) {
...
void CTxMemPool::removeForBlock(const std::vector<CTransactionRef>& vtx, unsigned int nBlockHeight)
{
    ...
    {
        txiter it = mapTxTxidIndex.find(tx->GetHash());
        if (it != mapTxTxidIndex.end()) {
            setEntries stage;
            stage.insert(it);
...
void CTxMemPool::check(const CCoinsViewCache& active_coins_tip, int64_t spendheight) const
{
    ...
    for (const auto& it : GetSortedDepthAndScore()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            indexed_transaction_set::const_iterator it2 = mapTxTxidIndex.find(txin.prevout.hash);
            if (it2 != mapTxTxidIndex.end()) {
        ...
        for (; iter != mapNextTx.end() && iter->first->hash == it->GetTx().GetHash(); ++iter) {
            txiter childit = mapTxTxidIndex.find(iter->second->GetHash());
            assert(childit != mapTxTxidIndex.end()); // mapNextTx points to in-mempool transactions
    ...
    for (auto it = mapNextTx.cbegin(); it != mapNextTx.cend(); it++) {
        ...
        indexed_transaction_set::const_iterator it2 = mapTxTxidIndex.find(hash);
        const CTransaction& tx = it2->GetTx();
        assert(it2 != mapTxTxidIndex.end());
        assert(&tx == it->second);
    }
...
bool CTxMemPool::CompareDepthAndScore(const uint256& hasha, const uint256& hashb, bool wtxid)
{
     ...
     */
    LOCK(cs);
    indexed_transaction_set::const_iterator j = wtxid ? get_iter_from_wtxid(hashb) : mapTxTxidIndex.find(hashb);
    if (j == mapTxTxidIndex.end()) return false;
    indexed_transaction_set::const_iterator i = wtxid ? get_iter_from_wtxid(hasha) : mapTxTxidIndex.find(hasha);
    if (i == mapTxTxidIndex.end()) return true;
...
std::vector<CTxMemPool::indexed_transaction_set::const_iterator> CTxMemPool::GetSortedDepthAndScore() const
{
    ...
    for (indexed_transaction_set::iterator mi = mapTxTxidIndex.begin(); mi != mapTxTxidIndex.end(); ++mi) {
        iters.push_back(mi);
    }
...
{
    AssertLockHeld(cs);
    const auto i = mapTxTxidIndex.find(txid);
    return i == mapTxTxidIndex.end() ? nullptr : &(*i);
}

CTransactionRef CTxMemPool::get(const uint256& hash) const
{
    LOCK(cs);
    indexed_transaction_set::const_iterator i = mapTxTxidIndex.find(hash);
    if (i == mapTxTxidIndex.end())
        return nullptr;
...
{
    LOCK(cs);
    indexed_transaction_set::const_iterator i = (gtxid.IsWtxid() ? get_iter_from_wtxid(gtxid.GetHash()) : mapTxTxidIndex.find(gtxid.GetHash()));
    if (i == mapTxTxidIndex.end())
...
TxMempoolInfo CTxMemPool::info_for_relay(const GenTxid& gtxid, uint64_t last_sequence) const
{
    LOCK(cs);
    indexed_transaction_set::const_iterator i = (gtxid.IsWtxid() ? get_iter_from_wtxid(gtxid.GetHash()) : mapTxTxidIndex.find(gtxid.GetHash()));
    if (i != mapTxTxidIndex.end() && i->GetSequence() < last_sequence) {
        return GetInfo(i);
    } else {
        return TxMempoolInfo();
    }
...
{
    {
        ...
        txiter it = mapTxTxidIndex.find(hash);
        if (it != mapTxTxidIndex.end()) {
            mapTxTxidIndex.modify(it, [&nFeeDelta](CTxMemPoolEntry& e) { e.UpdateModifiedFee(nFeeDelta); });
            // Now update all ancestors' modified fees with descendants
            auto ancestors{AssumeCalculateMemPoolAncestors(__func__, *it, Limits::NoLimits(), /*fSearchForParents=*/false)};
            for (txiter ancestorIt : ancestors) {
                mapTxTxidIndex.modify(ancestorIt, [=](CTxMemPoolEntry& e){ e.UpdateDescendantState(0, nFeeDelta, 0);});
            }
            ...
            for (txiter descendantIt : setDescendants) {
                mapTxTxidIndex.modify(descendantIt, [=](CTxMemPoolEntry& e){ e.UpdateAncestorState(0, nFeeDelta, 0, 0); });
            }
            ++nTransactionsUpdated;
        }
        if (delta == 0) {
            mapDeltas.erase(hash);
            LogPrintf("PrioritiseTransaction: %s (%sin mempool) delta cleared\n", hash.ToString(), it == mapTxTxidIndex.end() ? "not " : "");
        } else {
            LogPrintf("PrioritiseTransaction: %s (%sin mempool) fee += %s, new delta=%s\n",
                      hash.ToString(),
                      it == mapTxTxidIndex.end() ? "not " : "",
...
std::vector<CTxMemPool::delta_info> CTxMemPool::GetPrioritisedTransactions() const
{
    ...
    for (const auto& [txid, delta] : mapDeltas) {
        const auto iter{mapTxTxidIndex.find(txid)};
        const bool in_mempool{iter != mapTxTxidIndex.end()};
...
{
    auto it = mapTxTxidIndex.find(txid);
    if (it != mapTxTxidIndex.end()) return it;
    return std::nullopt;
}
    ...
    while (candidates.size()) {
        ...
        if (parents.size() == 0) {
            ...
            for (const CTxMemPoolEntry& i : parents) {
                candidates.push_back(mapTxTxidIndex.iterator_to(i));
            }
        }
    }
    return maximum;
    ...
    for (size_t i{0}; i < clustered_txs.size(); ++i) {
        ...
        for (const auto& entries : {tx_iter->GetMemPoolParentsConst(), tx_iter->GetMemPoolChildrenConst()}) {
            for (const CTxMemPoolEntry& entry : entries) {
                const auto entry_it = mapTxTxidIndex.iterator_to(entry);
                if (!visited(entry_it)) {
...
util::Result<std::pair<std::vector<FeeFrac>, std::vector<FeeFrac>>> CTxMemPool::CalculateChunksForRBF(CAmount replacement_fees, int64_t replacement_vsize, const setEntries& direct_conflicts, const setEntries& all_conflicts)
{
    ...
    for (auto direct_conflict : direct_conflicts) {
        ...
        if (direct_conflict->GetMemPoolParentsConst().size() > 0) {
            ...
            if (!all_conflicts.count(mapTxTxidIndex.iterator_to(parent))) {
