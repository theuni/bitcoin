  ...
  if test x$bitcoin_qt_got_major_vers == x5; then
    ...
    if test x$bitcoin_cv_static_qt == xyes; then
      ...
      if test x$TARGET_OS == xwindows; then
        ...
        _BITCOIN_QT_CHECK_STATIC_PLUGINS([Q_IMPORT_PLUGIN(QXcbIntegrationPlugin)],[-lqxcb -lxcb-static -lX11 -lX11-xcb -lxcb])
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK(AC_CHECK_LIB([pcre16] ,[main],,AC_MSG_WARN([libpcre not found. Assuming qt has it built-in])))
