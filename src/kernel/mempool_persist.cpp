...
bool LoadMempool(CTxMemPool& pool, const fs::path& load_path, Chainstate& active_chainstate, ImportMempoolOptions&& opts)
{
    ...
    if (file.IsNull()) {
        LogInstance().LogInfo("Failed to open mempool file. Continuing anyway.\n");
        return false;
    }
    ...
    try {
        ...
        LogInstance().LogInfo("Loading %u mempool transactions from file...\n", total_txns_to_load);
        ...
        while (txns_tried < total_txns_to_load) {
            ...
            if (next_tenth_to_report < percentage_done / 10) {
                LogInstance().LogInfo("Progress loading mempool transactions from file: %d%% (tried %u, %u remaining)\n",
        ...
        LogInstance().LogInfo("Failed to deserialize mempool data on file: %s. Continuing anyway.\n", e.what());
        return false;
    }

    LogInstance().LogInfo("Imported mempool transactions from file: %i succeeded, %i failed, %i expired, %i already there, %i waiting for initial broadcast\n", count, failed, expired, already_there, unbroadcast);
    return true;
}
...
bool DumpMempool(const CTxMemPool& pool, const fs::path& dump_path, FopenFn mockable_fopen_function, bool skip_file_commit)
{
    ...
    try {
        ...
        LogInstance().LogInfo("Writing %u mempool transactions to file...\n", mempool_transactions_to_write);
        ...
        LogInstance().LogInfo("Writing %d unbroadcast transactions to file.\n", unbroadcast_txids.size());
        file << unbroadcast_txids;
        ...
        LogInstance().LogInfo("Dumped mempool: %.3fs to copy, %.3fs to dump, %d bytes dumped to file\n",
                  Ticks<SecondsDouble>(mid - start),
                  Ticks<SecondsDouble>(last - mid),
                  fs::file_size(dump_path));
    } catch (const std::exception& e) {
        LogInstance().LogInfo("Failed to dump mempool: %s. Continuing anyway.\n", e.what());
        return false;
    }
    return true;
}
