...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    node.evictionman = std::make_unique<EvictionMan>(max_outbound_block_relay, max_outbound_full_relay);
    ...
    node.peerman = PeerManager::make(chainparams, *node.connman, *node.addrman, node.banman.get(),
                                     chainman, node.evictionman.get(), *node.mempool, ignores_incoming_txs);
    ...
    connOptions.m_evictionman = node.evictionman.get();
