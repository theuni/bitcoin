    ...
    for (bool fLoaded = false; !fLoaded && !ShutdownRequested();) {
        ...
        auto catch_exceptions = [](auto&& f) {
            try {
                ...
                LogPrintf("%s\n", e.what());
                return node::ChainstateLoadResult{node::ChainstateLoadStatus::FAILURE, _("Error opening block database")};
            }
        };
        node::ChainstateLoadResult result = catch_exceptions([&]{ return LoadChainstate(chainman, cache_sizes, options); });
        if (result.m_status == node::ChainstateLoadStatus::SUCCESS) {
        ...
        if (result.m_status == node::ChainstateLoadStatus::FAILURE_INCOMPATIBLE_DB || status == node::ChainstateLoadStatus::FAILURE_INSUFFICIENT_DBCACHE) {
            return InitError(error);
        }
        ...
        if (!fLoaded && !ShutdownRequested()) {
            ...
            if (!options.reindex) {
                bool fRet = uiInterface.ThreadSafeQuestion(
                    result.m_str + Untranslated(".\n\n") + _("Do you want to rebuild the block database now?"),
                    result.m_str.original + ".\nPlease restart with -reindex or -reindex-chainstate to recover.",
