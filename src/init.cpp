...
boost::shared_ptr<CConnman> g_connman;
...
bool static Bind(boost::shared_ptr<CConnman> connman, const CService &addr, unsigned int flags) {
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    assert(!g_connman);
    g_connman = boost::make_shared<CConnman>();
    ...
    if (mapArgs.count("-whitelist")) {
        BOOST_FOREACH(const std::string& net, mapMultiArgs["-whitelist"]) {
            ...
            g_connman->AddWhitelistedRange(subnet);
        }
    }
    ...
    if (fListen) {
        if (mapArgs.count("-bind") || mapArgs.count("-whitebind")) {
            BOOST_FOREACH(const std::string& strBind, mapMultiArgs["-bind"]) {
                ...
                fBound |= Bind(g_connman, addrBind, (BF_EXPLICIT | BF_REPORT_ERROR));
            }
            BOOST_FOREACH(const std::string& strBind, mapMultiArgs["-whitebind"]) {
                ...
                fBound |= Bind(g_connman, addrBind, (BF_EXPLICIT | BF_REPORT_ERROR | BF_WHITELIST));
            }
        }
        else {
            struct in_addr inaddr_any;
            inaddr_any.s_addr = INADDR_ANY;
            fBound |= Bind(g_connman, CService(in6addr_any, GetListenPort()), BF_NONE);
            fBound |= Bind(g_connman, CService(inaddr_any, GetListenPort()), !fBound ? BF_REPORT_ERROR : BF_NONE);
        }
        if (!fBound)
    ...
    if (mapArgs.count("-maxuploadtarget")) {
        g_connman->SetMaxOutboundTarget(GetArg("-maxuploadtarget", DEFAULT_MAX_UPLOAD_TARGET)*1024*1024);
    }
    ...
    if(!StartNode(g_connman, threadGroup, scheduler, nLocalServices, strNodeError))
