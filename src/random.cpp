...
{
    LogInstance().LogPrintf("Failed to read randomness, aborting\n");
    std::abort();
}
...
static void ReportHardwareRand()
{
    ...
    if (g_rdseed_supported) {
        LogInstance().LogPrintf("Using RdSeed as an additional entropy source\n");
    }
    if (g_rdrand_supported) {
        LogInstance().LogPrintf("Using RdRand as an additional entropy source\n");
    }
}
...
static void ReportHardwareRand()
{
    ...
    // from global constructors, before logging is initialized.
    if (g_rndr_supported) {
        LogInstance().LogPrintf("Using RNDR and RNDRRS as additional entropy sources\n");
    }
}
...
static void SeedPeriodic(CSHA512& hasher, RNGState& rng) noexcept
{
    ...
    LogInstance().LogPrint(BCLog::RAND, "Feeding %i bytes of dynamic environment data into RNG\n", hasher.Size() - old_size);
...
static void SeedStartup(CSHA512& hasher, RNGState& rng) noexcept
{
    ...
    LogInstance().LogPrint(BCLog::RAND, "Feeding %i bytes of environment data into RNG\n", hasher.Size() - old_size);
