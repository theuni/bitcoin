...
CNode* CConnman::ConnectNode(const CAddress& addrConnect, const char *pszDest, bool fCountFailure)
{
    ...
    // Connect
    CAddress addrResult = addrConnect;
    SOCKET hSocket;
...
{
    unsigned int nMessageSize = msg.data.size();
    unsigned int nTotalSize = nMessageSize + CMessageHeader::HEADER_SIZE;
    LogPrint("net", "sending %s (%d bytes) peer=%d\n",  SanitizeString(msg.command.c_str()), nTotalSize, pnode->id);

    // TODO: Maybe just add 2 messages for each push, one header and one payload.
    uint256 hash = Hash(msg.data.data(), msg.data.data() + msg.data.size());
    CMessageHeader hdr(Params().MessageStart(), msg.command.c_str(), nMessageSize);
    memcpy(hdr.pchChecksum, hash.begin(), CMessageHeader::CHECKSUM_SIZE);

    std::vector<unsigned char> finalmsg;
    finalmsg.reserve(nTotalSize);
    CVectorWriter writer{SER_NETWORK, INIT_PROTO_VERSION, finalmsg};


    ::Serialize(writer, hdr);
    assert(finalmsg.size() == CMessageHeader::HEADER_SIZE);
    if(!msg.data.empty()) {
        finalmsg.insert(finalmsg.end(), msg.data.begin(), msg.data.end());
    }
    finalmsg.shrink_to_fit();
    ...
    size_t nBytesSent = 0;
    {
        ...
        pnode->vSendMsg.push_back(std::move(finalmsg));
