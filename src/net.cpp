...
[[nodiscard]] std::optional<CService> GetLocalAddress(const CAddress& addr, const Network& network)
{
...
std::optional<CService> GetLocalAddrForPeer(CNode& node)
{
    ...
    // one by discovery.
    CService addrLocal{GetLocalAddress(node.addr, node.ConnectedThroughNetwork()).value_or(CService{CNetAddr(), GetListenPort()})};
...
void Discover()
{
    ...
    for (const CNetAddr &addr: GetLocalAddresses()) {
        if (AddLocal(CService(addr, GetListenPort()), LOCAL_IF))
