...
uint256 ParseHashUV(const UniValue& v, string strName)
{
    string strHex;
    if (v.isStr())
        strHex = v.getValStr();
...
vector<unsigned char> ParseHexUV(const UniValue& v, string strName)
{
    string strHex;
    if (v.isStr())
        strHex = v.getValStr();
                     ...
                     const string& commandVal)
{
    if (command == "nversion")
        ...
        MutateTxSign(tx, commandVal);
             ...
             (command.substr(0, 5) == "load."))
        RegisterLoad(command.substr(5), commandVal);

    // fallback: if unknown command, with a value, assume register assignment
    else if (commandVal.size() > 0)
        RegisterSet(command, commandVal);
