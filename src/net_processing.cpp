...
void PeerManagerImpl::MaybeSetPeerAsAnnouncingHeaderAndIDs(NodeId nodeid)
{
    ...
    if (!peer->m_disconnecting && peer->m_handshake_complete) {
        AssertLockHeld(::cs_main);
        if (lNodesAnnouncingHeaderAndIDs.size() >= 3) {
            // As per BIP152, we only get 3 of our peers to announce
            // blocks using compact encodings.
            auto peerStop = GetPeerRef(lNodesAnnouncingHeaderAndIDs.front());
            if (!peerStop->m_disconnecting && peerStop->m_handshake_complete) {
            ...
            if (peer->HasPermission(NetPermissionFlags::ForceRelay)) {
                ...
                if (!m_mempool.exists(GenTxid::Txid(tx.GetHash()))) {
                    LogPrintf("Not relaying non-mempool transaction %s (wtxid=%s) from forcerelay peer=%d\n",
                              tx.GetHash().ToString(), tx.GetWitnessHash().ToString(), node_id);
                } else {
...
void PeerManagerImpl::EvictExtraOutboundPeers(std::chrono::seconds now)
{
    ...
    if (m_connman.GetExtraFullOutboundCount() > 0) {
        ...
        ForEachFullyConnectedPeer([&](const Peer& peer) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
            ...
            if(WITH_LOCK(m_peer_mutex, return m_network_conn_counts[peer.m_addr.GetNetwork()] > 1)) return;
