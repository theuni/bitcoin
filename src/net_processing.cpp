...
void PeerManagerImpl::RemoveBlockRequest(const uint256& hash)
{
    ...
    if (m_evictor) {
        m_evictor->UpdateBlocksInFlight(node_id, false);
    }
...
bool PeerManagerImpl::BlockRequested(NodeId nodeid, const CBlockIndex& block, std::list<QueuedBlock>::iterator** pit)
{
    ...
    state->nBlocksInFlight++;
    if (m_evictor) {
        m_evictor->UpdateBlocksInFlight(nodeid, true);
    }
                                            ...
                                            bool via_compact_block)
{
...

    {
        ...
        if (received_new_header && pindexLast->nChainWork > m_chainman.ActiveChain().Tip()->nChainWork) {
            ...
            if (m_evictor) {
                m_evictor->UpdateLastBlockAnnouncementTime(pfrom.GetId(), announce_time);
            }
        }
        ...
        if (!pfrom.fDisconnect && pfrom.IsFullOutboundConn() && nodestate->pindexBestKnownBlock != nullptr) {
            if (m_outbound_peers_with_protect_from_disconnect < MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT && nodestate->pindexBestKnownBlock->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork && !nodestate->m_chain_sync.m_protect) {
                ...
                if (m_evictor) {
                    m_evictor->UpdateSlowChainProtected(pfrom.GetId(), true);
                }
            }
        }
    }
...
void PeerManagerImpl::ProcessBlock(CNode& node, const std::shared_ptr<const CBlock>& block, bool force_processing)
{
    ...
    if (new_block) {
        ...
        node.nLastBlockTime = curtime;
        if (m_evictor) {
            m_evictor->UpdateLatestBlockTime(node.GetId(), curtime);
        }
    } else {
        LOCK(cs_main);
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            if (m_evictor) {
                m_evictor->UpdateLatestTxTime(pfrom.GetId(), curtime);
            }
...
void PeerManagerImpl::CheckForStaleTipAndEvictPeers()
{
    ...
    if (auto nodeid = EvictExtraBlockOutboundPeers(time_in_seconds); nodeid.has_value()) {
        m_connman.DisconnectNode(nodeid.value());
    }
    if (auto nodeid = EvictExtraFullOutboundPeers(time_in_seconds); nodeid.has_value()) {
        m_connman.DisconnectNode(nodeid.value());
