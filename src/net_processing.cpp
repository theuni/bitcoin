...
public:
    PeerManagerImpl(const CChainParams& chainparams, CConnman& connman, AddrMan& addrman,
                    BanMan* banman, ChainstateManager& chainman, EvictionMan* evictionman,
...
std::unique_ptr<PeerManager> PeerManager::make(const CChainParams& chainparams, CConnman& connman, AddrMan& addrman,
                                               ...
                                               CTxMemPool& pool, bool ignore_incoming_txs)
{
    return std::make_unique<PeerManagerImpl>(chainparams, connman, addrman, banman, chainman, evictionman, pool, ignore_incoming_txs);
}
...
void PeerManagerImpl::ConsiderEviction(CNode& pto, int64_t time_in_seconds)
{
    ...
    if (!state.m_chain_sync.m_protect && pto.IsOutboundOrBlockRelayConn() && state.fSyncStarted) {
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork) {
            ...
            state.m_chain_sync.m_sent_getheaders = false;
        } else if (state.m_chain_sync.m_work_header != nullptr && state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= state.m_chain_sync.m_work_header->nChainWork) {
            // This peer was able to catch up to some earlier point
...
void PeerManagerImpl::CheckForStaleTipAndEvictPeers()
{
    ...
    if (m_evictionman) {
        auto&& [nodes_to_disconnect, disconnected] = m_evictionman->EvictExtraOutboundPeers(time_in_seconds);
        for (const auto id : nodes_to_disconnect) {
            m_connman.DisconnectNode(id);
        }
        if (disconnected) {
