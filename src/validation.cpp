    ...
    const CTransaction& tx)
{
    ...
    for (size_t i = 0; i < tx.vin.size(); ++i) {
        ...
        if (!coins.GetCoin(txin.prevout, coin)) {
            LogInstance().LogPrintf("ERROR: %s: Missing input %d in transaction \'%s\'\n", __func__, i, tx.GetHash().GetHex());
            return std::nullopt;
        }
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main, pool.cs)
{
    ...
    if (expired != 0) {
        LogInstance().LogPrint(BCLog::MEMPOOL, "Expired %i transactions from the memory pool\n", expired);
    }
                                         ...
                                         PackageValidationState& package_state)
{
    ...
    LogInstance().LogPrint(BCLog::TXPACKAGES, "package RBF checks passed: parent %s (wtxid=%s), child %s (wtxid=%s)\n",
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckInputsFromMempoolAndCache(tx, state, m_view, m_pool, currentBlockScriptVerifyFlags,
        ...
        LogInstance().LogPrintf("BUG! PLEASE REPORT THIS! CheckInputScripts failed against latest-block but not STANDARD flags %s, %s\n", hash.ToString(), state.ToString());
        return Assume(false);
    }
...
bool MemPoolAccept::Finalize(const ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : m_subpackage.m_all_conflicts)
    {
        LogInstance().LogPrint(BCLog::MEMPOOL, "replacing mempool tx %s (wtxid=%s, fees=%s, vsize=%s). New tx %s (wtxid=%s, fees=%s, vsize=%s)\n",
                                  ...
                                  std::map<uint256, MempoolAcceptResult>& results)
{
    ...
    if (!m_subpackage.m_replaced_transactions.empty()) {
        LogInstance().LogPrint(BCLog::MEMPOOL, "replaced %u mempool transactions with %u new one(s) for %s additional fees, %d delta bytes\n",
...
MempoolAcceptResult MemPoolAccept::AcceptSingleTransaction(const CTransactionRef& ptx, ATMPArgs& args)
{
    ...
    if (!m_subpackage.m_replaced_transactions.empty()) {
        LogInstance().LogPrint(BCLog::MEMPOOL, "replaced %u mempool transactions with 1 new transaction for %s additional fees, %d delta bytes\n",
...
bool ChainstateManager::IsInitialBlockDownload() const
{
    ...
    LogInstance().LogPrintf("Leaving InitialBlockDownload (latching to false)\n");
...
void Chainstate::CheckForkWarningConditions()
{
    ...
    if (m_chainman.m_best_invalid && m_chainman.m_best_invalid->nChainWork > m_chain.Tip()->nChainWork + (GetBlockProof(*m_chain.Tip()) * 6)) {
        LogInstance().LogPrintf("%s: Warning: Found invalid chain at least ~6 blocks longer than our best chain.\nChain state database corruption likely.\n", __func__);
...
void Chainstate::InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogInstance().LogPrintf("%s: invalid block=%s  height=%d  log2_work=%f  date=%s\n", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->nHeight,
      log(pindexNew->nChainWork.getdouble())/log(2.0), FormatISO8601DateTime(pindexNew->GetBlockTime()));
    CBlockIndex *tip = m_chain.Tip();
    assert (tip);
    LogInstance().LogPrintf("%s:  current best=%s  height=%d  log2_work=%f  date=%s\n", __func__,
...
bool InitScriptExecutionCache(size_t max_size_bytes)
{
    ...
    LogInstance().LogPrintf("Using %zu MiB out of %zu MiB requested for script execution cache, able to store %zu elements\n",
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!m_blockman.UndoReadFromDisk(blockUndo, *pindex)) {
        LogInstance().LogError("DisconnectBlock(): failure reading undo data\n");
        return DISCONNECT_FAILED;
    }

    if (blockUndo.vtxundo.size() + 1 != block.vtx.size()) {
        LogInstance().LogError("DisconnectBlock(): block and undo data inconsistent\n");
        return DISCONNECT_FAILED;
    }
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (i > 0) { // not coinbases
            ...
            if (txundo.vprevout.size() != tx.vin.size()) {
                LogInstance().LogError("DisconnectBlock(): transaction and undo data inconsistent\n");
                return DISCONNECT_FAILED;
            }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlock(block, state, params.GetConsensus(), !fJustCheck, !fJustCheck)) {
        ...
        LogInstance().LogError("%s: Consensus::CheckBlock: %s\n", __func__, state.ToString());
        return false;
    }
    ...
    LogInstance().LogPrint(BCLog::BENCH, "    - Sanity checks: %.2fms [%.2fs (%.2fms/blk)]\n",
        ...
        for (const auto& tx : block.vtx) {
            for (size_t o = 0; o < tx->vout.size(); o++) {
                if (view.HaveCoin(COutPoint(tx->GetHash(), o))) {
                    LogInstance().LogPrintf("ERROR: ConnectBlock(): tried to overwrite transaction\n");
    ...
    LogInstance().LogPrint(BCLog::BENCH, "    - Fork checks: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!Consensus::CheckTxInputs(tx, tx_state, view, pindex->nHeight, txfee)) {
                ...
                LogInstance().LogError("%s: Consensus::CheckTxInputs: %s, %s\n", __func__, tx.GetHash().ToString(), state.ToString());
                return false;
            }
            nFees += txfee;
            if (!MoneyRange(nFees)) {
                LogInstance().LogPrintf("ERROR: %s: accumulated fee in the block out of range.\n", __func__);
            ...
            if (!SequenceLocks(tx, nLockTimeFlags, prevheights, *pindex)) {
                LogInstance().LogPrintf("ERROR: %s: contains a non-BIP68-final transaction\n", __func__);
        ...
        if (nSigOpsCost > MAX_BLOCK_SIGOPS_COST) {
            LogInstance().LogPrintf("ERROR: ConnectBlock(): too many sigops\n");
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (fScriptChecks && !CheckInputScripts(tx, tx_state, view, flags, fCacheResults, fCacheResults, txsdata[i], parallel_script_checks ? &vChecks : nullptr)) {
                ...
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                              tx_state.GetRejectReason(), tx_state.GetDebugMessage());
                LogInstance().LogError("ConnectBlock(): CheckInputScripts on %s failed with %s\n",
    ...
    LogInstance().LogPrint(BCLog::BENCH, "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs (%.2fms/blk)]\n", (unsigned)block.vtx.size(),
    ...
    if (block.vtx[0]->GetValueOut() > blockReward) {
        LogInstance().LogPrintf("ERROR: ConnectBlock(): coinbase pays too much (actual=%d vs limit=%d)\n", block.vtx[0]->GetValueOut(), blockReward);
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount");
    }

    if (!control.Wait()) {
        LogInstance().LogPrintf("ERROR: %s: CheckQueue failed\n", __func__);
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "block-validation-failed");
    }
    const auto time_4{SteadyClock::now()};
    time_verify += time_4 - time_2;
    LogInstance().LogPrint(BCLog::BENCH, "    - Verify %u txins: %.2fms (%.3fms/txin) [%.2fs (%.2fms/blk)]\n", nInputs - 1,
    ...
    LogInstance().LogPrint(BCLog::BENCH, "    - Write undo data: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    LogInstance().LogPrint(BCLog::BENCH, "    - Index writing: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    size_t max_mempool_size_bytes)
{
    ...
    if (cacheSize > nTotalSpace) {
        LogInstance().LogPrintf("Cache size (%s) exceeds total space (%s)\n", cacheSize, nTotalSpace);
    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        if (m_blockman.IsPruneMode() && (m_blockman.m_check_for_pruning || nManualPruneHeight > 0) && m_chainman.m_blockman.m_blockfiles_indexed) {
            ...
            if (limiting_lock) {
                LogInstance().LogPrint(BCLog::PRUNE, "%s limited pruning to height %d\n", limiting_lock.value(), last_prune);
            }
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            }
            {
                ...
                if (!m_blockman.FlushChainstateBlockFile(m_chain.Height())) {
                    LogInstance().LogPrintLevel(BCLog::VALIDATION, BCLog::Level::Warning, "%s: Failed to flush block file.\n", __func__);
                }
            }
...
void Chainstate::ForceFlushStateToDisk()
{
    ...
    if (!this->FlushStateToDisk(state, FlushStateMode::ALWAYS)) {
        LogInstance().LogPrintf("%s: failed to flush state (%s)\n", __func__, state.ToString());
    }
}
    ...
    const std::string& warning_messages) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    AssertLockHeld(::cs_main);
    LogInstance().LogPrintf("%s%s: new best=%s height=%d version=0x%08x log2_work=%f tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)%s\n",
        prefix, func_name,
...
bool Chainstate::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    if (!m_blockman.ReadBlockFromDisk(block, *pindexDelete)) {
        LogInstance().LogError("DisconnectTip(): Failed to read block\n");
        return false;
    }
    ...
    const auto time_start{SteadyClock::now()};
    {
        ...
        if (DisconnectBlock(block, pindexDelete, view) != DISCONNECT_OK) {
            LogInstance().LogError("DisconnectTip(): DisconnectBlock %s failed\n", pindexDelete->GetBlockHash().ToString());
            return false;
        }
        bool flushed = view.Flush();
        assert(flushed);
    }
    LogInstance().LogPrint(BCLog::BENCH, "- Disconnect block: %.2fms\n",
...

    {
        ...
        for (auto& prune_lock : m_blockman.m_prune_locks) {
            ...
            LogInstance().LogPrint(BCLog::PRUNE, "%s prune lock moved back to %d\n", prune_lock.first, max_height_first);
        }
    }
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    if (!pblock) {
        ...
        LogInstance().LogPrint(BCLog::BENCH, "  - Using cached block\n");
        pthisBlock = pblock;
    }
    ...
    LogInstance().LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms\n",
             Ticks<MillisecondsDouble>(time_2 - time_1));
    {
        CCoinsViewCache view(&CoinsTip());
        ...
        if (!rv) {
            ...
            LogInstance().LogError("%s: ConnectBlock %s failed, %s\n", __func__, pindexNew->GetBlockHash().ToString(), state.ToString());
            return false;
        }
        time_3 = SteadyClock::now();
        time_connect_total += time_3 - time_2;
        assert(num_blocks_total > 0);
        LogInstance().LogPrint(BCLog::BENCH, "  - Connect total: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    LogInstance().LogPrint(BCLog::BENCH, "  - Flush: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    LogInstance().LogPrint(BCLog::BENCH, "  - Writing chainstate: %.2fms [%.2fs (%.2fms/blk)]\n",
             Ticks<MillisecondsDouble>(time_5 - time_4),
    ...
    LogInstance().LogPrint(BCLog::BENCH, "  - Connect postprocess: %.2fms [%.2fs (%.2fms/blk)]\n",
             Ticks<MillisecondsDouble>(time_6 - time_5),
             Ticks<SecondsDouble>(time_post_connect),
             Ticks<MillisecondsDouble>(time_post_connect) / num_blocks_total);
    LogInstance().LogPrint(BCLog::BENCH, "- Connect block: %.2fms [%.2fs (%.2fms/blk)]\n",
...
bool Chainstate::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
    ...
    if (WITH_LOCK(::cs_main, return m_disabled)) {
        LogInstance().LogPrintf("m_disabled is set - this chainstate should not be in operation. "
...
void ChainstateManager::ReceivedBlockTransactions(const CBlock& block, CBlockIndex* pindexNew, const FlatFilePos& pos)
{
    ...
    if (!Assume(pindexNew->nChainTx == 0 || pindexNew->nChainTx == prev_tx_sum(*pindexNew) ||
        ...
        LogInstance().LogWarning("Internal bug detected: block %d has unexpected nChainTx %i that should be %i (%s %s). Please report this issue here: %s\n",
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (!CheckMerkleRoot(block, state)) {
        LogInstance().LogDebug(BCLog::VALIDATION, "Block mutated: %s\n", state.ToString());
        return true;
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (chainman.m_options.checkpoints_enabled) {
        ...
        if (pcheckpoint && nHeight < pcheckpoint->nHeight) {
            LogInstance().LogPrintf("ERROR: %s: forked chain older than last checkpoint (height %d)\n", __func__, nHeight);
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        if (miSelf != m_blockman.m_block_index.end()) {
            ...
            if (ppindex)
                *ppindex = pindex;
            if (pindex->nStatus & BLOCK_FAILED_MASK) {
                LogInstance().LogPrint(BCLog::VALIDATION, "%s: block %s is marked invalid\n", __func__, hash.ToString());
        ...
        if (!CheckBlockHeader(block, state, GetConsensus())) {
            LogInstance().LogPrint(BCLog::VALIDATION, "%s: Consensus::CheckBlockHeader: %s, %s\n", __func__, hash.ToString(), state.ToString());
            return false;
        }
        ...
        if (mi == m_blockman.m_block_index.end()) {
            LogInstance().LogPrint(BCLog::VALIDATION, "header %s has prev block not found: %s\n", hash.ToString(), block.hashPrevBlock.ToString());
            return state.Invalid(BlockValidationResult::BLOCK_MISSING_PREV, "prev-blk-not-found");
        }
        pindexPrev = &((*mi).second);
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            LogInstance().LogPrint(BCLog::VALIDATION, "header %s has prev block invalid: %s\n", hash.ToString(), block.hashPrevBlock.ToString());
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_PREV, "bad-prevblk");
        }
        if (!ContextualCheckBlockHeader(block, state, m_blockman, *this, pindexPrev)) {
            LogInstance().LogPrint(BCLog::VALIDATION, "%s: Consensus::ContextualCheckBlockHeader: %s, %s\n", __func__, hash.ToString(), state.ToString());
            return false;
        }
    ...
    if (!min_pow_checked) {
        LogInstance().LogPrint(BCLog::VALIDATION, "%s: not adding new block header %s, missing anti-dos proof-of-work validation\n", __func__, hash.ToString());
    ...
    if (IsInitialBlockDownload()) {
        LogInstance().LogPrintLevel(BCLog::VALIDATION, BCLog::Level::Debug, "%s\n", msg);
    } else {
        LogInstance().LogPrintf("%s\n", msg);
    }
...
bool ChainstateManager::ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, bool min_pow_checked, BlockValidationState& state, const CBlockIndex** ppindex)
{
    ...
    if (NotifyHeaderTip(*this)) {
        if (IsInitialBlockDownload() && ppindex && *ppindex) {
            ...
            LogInstance().LogInfo("Synchronizing blockheaders, height: %d (~%.2f%%)\n", last_accepted.nHeight, progress);
        }
    }
    return true;
}
...
void ChainstateManager::ReportHeadersPresync(const arith_uint256& work, int64_t height, int64_t timestamp)
{
    ...
    if (initial_download) {
        ...
        LogInstance().LogInfo("Pre-synchronizing blockheaders, height: %d (~%.2f%%)\n", height, progress);
    }
}
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        if (!ret) {
            ...
            LogInstance().LogError("%s: AcceptBlock FAILED (%s)\n", __func__, state.ToString());
            return false;
        }
    }
    ...
    if (!ActiveChainstate().ActivateBestChain(state, block)) {
        LogInstance().LogError("%s: ActivateBestChain failed (%s)\n", __func__, state.ToString());
        return false;
    }
    ...
    if (bg_chain && !bg_chain->ActivateBestChain(bg_state, block)) {
        LogInstance().LogError("%s: [background] ActivateBestChain failed (%s)\n", __func__, bg_state.ToString());
        return false;
     }
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    if (!ContextualCheckBlockHeader(block, state, chainstate.m_blockman, chainstate.m_chainman, pindexPrev)) {
        LogInstance().LogError("%s: Consensus::ContextualCheckBlockHeader: %s\n", __func__, state.ToString());
        return false;
    }
    ...
    if (!ContextualCheckBlock(block, state, chainstate.m_chainman, pindexPrev)) {
        LogInstance().LogError("%s: Consensus::ContextualCheckBlock: %s\n", __func__, state.ToString());
        return false;
    }
...
bool Chainstate::LoadChainTip()
{
    ...
    tip = m_chain.Tip();
    LogInstance().LogPrintf("Loaded best chain: hashBestChain=%s height=%d date=%s progress=%f\n",
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    LogInstance().LogPrintf("Verifying last %i blocks at level %i\n", nCheckDepth, nCheckLevel);
    ...
    LogInstance().LogPrintf("Verification progress: 0%%\n");
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (reportDone < percentageDone / 10) {
            ...
            LogInstance().LogPrintf("Verification progress: %d%%\n", percentageDone);
        ...
        if ((chainstate.m_blockman.IsPruneMode() || is_snapshot_cs) && !(pindex->nStatus & BLOCK_HAVE_DATA)) {
            ...
            LogInstance().LogPrintf("VerifyDB(): block verification stopping at height %d (no data). This could be due to pruning or use of an assumeutxo snapshot.\n", pindex->nHeight);
        ...
        if (!chainstate.m_blockman.ReadBlockFromDisk(block, *pindex)) {
            LogInstance().LogPrintf("Verification error: ReadBlockFromDisk failed at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
            return VerifyDBResult::CORRUPTED_BLOCK_DB;
        }
        // check level 1: verify block validity
        if (nCheckLevel >= 1 && !CheckBlock(block, state, consensus_params)) {
            LogInstance().LogPrintf("Verification error: found bad block at %d, hash=%s (%s)\n",
        ...
        if (nCheckLevel >= 2 && pindex) {
            ...
            if (!pindex->GetUndoPos().IsNull()) {
                if (!chainstate.m_blockman.UndoReadFromDisk(undo, *pindex)) {
                    LogInstance().LogPrintf("Verification error: found bad undo data at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                    return VerifyDBResult::CORRUPTED_BLOCK_DB;
                }
            }
        }
        ...
        if (nCheckLevel >= 3) {
            if (curr_coins_usage <= chainstate.m_coinstip_cache_size_bytes) {
                ...
                if (res == DISCONNECT_FAILED) {
                    LogInstance().LogPrintf("Verification error: irrecoverable inconsistency in block data at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                    return VerifyDBResult::CORRUPTED_BLOCK_DB;
                }
    ...
    if (pindexFailure) {
        LogInstance().LogPrintf("Verification error: coin database inconsistencies found (last %i blocks, %i good transactions before that)\n", chainstate.m_chain.Height() - pindexFailure->nHeight + 1, nGoodTransactions);
        return VerifyDBResult::CORRUPTED_BLOCK_DB;
    }
    if (skipped_l3_checks) {
        LogInstance().LogPrintf("Skipped verification of level >=3 (insufficient database cache size). Consider increasing -dbcache.\n");
    }
    ...
    if (nCheckLevel >= 4 && !skipped_l3_checks) {
        while (pindex != chainstate.m_chain.Tip()) {
            ...
            if (!chainstate.ConnectBlock(block, state, pindex, coins)) {
                LogInstance().LogPrintf("Verification error: found unconnectable block at %d, hash=%s (%s)\n", pindex->nHeight, pindex->GetBlockHash().ToString(), state.ToString());
                return VerifyDBResult::CORRUPTED_BLOCK_DB;
            }
            if (chainstate.m_chainman.m_interrupt) return VerifyDBResult::INTERRUPTED;
        }
    ...
    LogInstance().LogPrintf("Verification: No coin database inconsistencies in last %i blocks (%i transactions)\n", block_count, nGoodTransactions);
...
bool Chainstate::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    if (!m_blockman.ReadBlockFromDisk(block, *pindex)) {
        LogInstance().LogError("ReplayBlock(): ReadBlockFromDisk failed at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
        return false;
    }
...
bool Chainstate::ReplayBlocks()
{
    ...
    if (hashHeads.size() != 2) {
        LogInstance().LogError("ReplayBlocks(): unknown inconsistent state\n");
        return false;
    }

    m_chainman.GetNotifications().progress(_("Replaying blocks…"), 0, false);
    LogInstance().LogPrintf("Replaying blocks\n");
    ...
    if (m_blockman.m_block_index.count(hashHeads[0]) == 0) {
        LogInstance().LogError("ReplayBlocks(): reorganization to unknown block requested\n");
        return false;
    }
    ...
    if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating it's the first flush.
        if (m_blockman.m_block_index.count(hashHeads[1]) == 0) {
            LogInstance().LogError("ReplayBlocks(): reorganization from unknown block requested\n");
            return false;
        }
    ...
    while (pindexOld != pindexFork) {
        if (pindexOld->nHeight > 0) { // Never disconnect the genesis block.
            ...
            if (!m_blockman.ReadBlockFromDisk(block, *pindexOld)) {
                LogInstance().LogError("RollbackBlock(): ReadBlockFromDisk() failed at %d, hash=%s\n", pindexOld->nHeight, pindexOld->GetBlockHash().ToString());
                return false;
            }
            LogInstance().LogPrintf("Rolling back %s (%i)\n", pindexOld->GetBlockHash().ToString(), pindexOld->nHeight);
            DisconnectResult res = DisconnectBlock(block, pindexOld, cache);
            if (res == DISCONNECT_FAILED) {
                LogInstance().LogError("RollbackBlock(): DisconnectBlock failed at %d, hash=%s\n", pindexOld->nHeight, pindexOld->GetBlockHash().ToString());
                return false;
            }
    ...
    for (int nHeight = nForkHeight + 1; nHeight <= pindexNew->nHeight; ++nHeight) {
        ...
        LogInstance().LogPrintf("Rolling forward %s (%i)\n", pindex.GetBlockHash().ToString(), nHeight);
...
bool Chainstate::LoadGenesisBlock()
{
    ...
    try {
        ...
        if (blockPos.IsNull()) {
            LogInstance().LogError("%s: writing genesis block to disk failed\n", __func__);
            return false;
        }
        CBlockIndex* pindex = m_blockman.AddToBlockIndex(block, m_chainman.m_best_header);
        m_chainman.ReceivedBlockTransactions(block, pindex, blockPos);
    } catch (const std::runtime_error& e) {
        LogInstance().LogError("%s: failed to write genesis block: %s\n", __func__, e.what());
        return false;
    }
            ...
            try {
...

                {
                    ...
                    if (hash != params.GetConsensus().hashGenesisBlock && !m_blockman.LookupBlockIndex(header.hashPrevBlock)) {
                        LogInstance().LogPrint(BCLog::REINDEX, "%s: Out of order block %s, parent %s not known\n", __func__, hash.ToString(),
                    ...
                    if (!pindex || (pindex->nStatus & BLOCK_HAVE_DATA) == 0) {
                        ...
                        LogInstance().LogPrint(BCLog::REINDEX, "Block Import: already had block %s at height %d\n", hash.ToString(), pindex->nHeight);
                    }
                }
                ...
                if (m_blockman.IsPruneMode() && m_blockman.m_blockfiles_indexed && pblock) {
                    ...
                    for (auto c : GetAll()) {
                        ...
                        if (!c->ActivateBestChain(state, pblock)) {
                            LogInstance().LogPrint(BCLog::REINDEX, "failed to activate chain (%s)\n", state.ToString());
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (m_blockman.ReadBlockFromDisk(*pblockrecursive, it->second)) {
                            LogInstance().LogPrint(BCLog::REINDEX, "%s: Processing out of order child %s of %s\n", __func__, pblockrecursive->GetHash().ToString(),
                                    head.ToString());
                            LOCK(cs_main);
                ...
                LogInstance().LogPrint(BCLog::REINDEX, "%s: unexpected data at file offset 0x%x - %s. continuing\n", __func__, (nRewind - 1), e.what());
            }
        }
    } catch (const std::runtime_error& e) {
        GetNotifications().fatalError(strprintf(_("System error while loading external block file: %s"), e.what()));
    }
    LogInstance().LogPrintf("Loaded %i blocks from external file in %dms\n", nLoaded, Ticks<std::chrono::milliseconds>(SteadyClock::now() - start));
}
...
bool Chainstate::ResizeCoinsCaches(size_t coinstip_size, size_t coinsdb_size)
{
    ...
    LogInstance().LogPrintf("[%s] resized coinsdb cache to %.1f MiB\n",
        this->ToString(), coinsdb_size * (1.0 / 1024 / 1024));
    LogInstance().LogPrintf("[%s] resized coinstip cache to %.1f MiB\n",
...
double GuessVerificationProgress(const ChainTxData& data, const CBlockIndex *pindex) {
    ...
    if (!Assume(pindex->nChainTx > 0)) {
        LogInstance().LogWarning("Internal bug detected: block %d has unset nChainTx (%s %s). Please report this issue here: %s\n",
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (is_snapshot) {
        ...
        try {
            ...
            if (!existed) {
                LogInstance().LogPrintf("[snapshot] snapshot chainstate dir being removed lacks %s file\n",
                          fs::PathToString(node::SNAPSHOT_BLOCKHASH_FILENAME));
            }
        } catch (const fs::filesystem_error& e) {
            LogInstance().LogPrintf("[snapshot] failed to remove file %s: %s\n",
                    fs::PathToString(base_blockhash_path), fsbridge::get_filesystem_error_message(e));
        }
    }

    std::string path_str = fs::PathToString(db_path);
    LogInstance().LogPrintf("Removing leveldb dir at %s\n", path_str);
    ...
    if (!destroyed) {
        LogInstance().LogPrintf("error: leveldb DestroyDB call failed on %s\n", path_str);
    }
        ...
        bool in_memory)
{
    ...
    if (this->SnapshotBlockhash()) {
        LogInstance().LogPrintf("[snapshot] can't activate a snapshot-based chainstate more than once\n");
        return false;
    }

    {
        ...
        if (Assert(m_active_chainstate->GetMempool())->size() > 0) {
            LogInstance().LogPrintf("[snapshot] can't activate a snapshot when mempool not empty\n");
            return false;
        }
    }
    ...
    auto cleanup_bad_snapshot = [&](const char* reason) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        LogInstance().LogPrintf("[snapshot] activation failed - %s\n", reason);
    ...
    LogInstance().LogPrintf("[snapshot] successfully activated snapshot %s\n", base_blockhash.ToString());
    LogInstance().LogPrintf("[snapshot] (%.2f MB)\n",
    ...
    const SnapshotMetadata& metadata)
{
    ...
    if (!snapshot_start_block) {
        ...
        LogInstance().LogPrintf("[snapshot] Did not find snapshot start blockheader %s\n",
                  base_blockhash.ToString());
        return false;
    }
    ...
    if (!maybe_au_data) {
        LogInstance().LogPrintf("[snapshot] assumeutxo height in snapshot metadata not recognized "
    ...
    if (WITH_LOCK(::cs_main, return !CBlockIndexWorkComparator()(ActiveTip(), snapshot_start_block))) {
        LogInstance().LogPrintf("[snapshot] activation failed - work does not exceed active chainstate\n");
        return false;
    }
    ...
    LogInstance().LogPrintf("[snapshot] loading %d coins from snapshot %s\n", coins_left, base_blockhash.ToString());
    ...
    while (coins_left > 0) {
        try {
            ...
            if (coins_per_txid > coins_left) {
                LogInstance().LogPrintf("[snapshot] mismatch in coins count in snapshot metadata and actual snapshot data\n");
                return false;
            }
            ...
            for (size_t i = 0; i < coins_per_txid; i++) {
                ...
                if (coin.nHeight > base_height ||
                    ...
                    LogInstance().LogPrintf("[snapshot] bad snapshot data after deserializing %d coins\n",
                              coins_count - coins_left);
                    return false;
                }
                if (!MoneyRange(coin.out.nValue)) {
                    LogInstance().LogPrintf("[snapshot] bad snapshot data after deserializing %d coins - bad tx out value\n",
                              coins_count - coins_left);
                    return false;
                }
                ...
                if (coins_processed % 1000000 == 0) {
                    LogInstance().LogPrintf("[snapshot] %d coins loaded (%.2f%%, %.2f MB)\n",
                        coins_processed,
            ...
            LogInstance().LogPrintf("[snapshot] bad snapshot format or truncated snapshot after deserializing %d coins\n",
                      coins_processed);
            return false;
        }
    }
    ...
    if (!out_of_coins) {
        LogInstance().LogPrintf("[snapshot] bad snapshot - coins left over after deserializing %d coins\n",
            coins_count);
        return false;
    }

    LogInstance().LogPrintf("[snapshot] loaded %d (%.2f MB) coins from snapshot %s\n",
        coins_count,
        coins_cache.DynamicMemoryUsage() / (1000 * 1000),
        base_blockhash.ToString());
    ...
    if (!maybe_stats.has_value()) {
        LogInstance().LogPrintf("[snapshot] failed to generate coins stats\n");
        return false;
    }

    // Assert that the deserialized chainstate contents match the expected assumeutxo value.
    if (AssumeutxoHash{maybe_stats->hashSerialized} != au_data.hash_serialized) {
        LogInstance().LogPrintf("[snapshot] bad snapshot content hash: expected %s, got %s\n",
    ...
    LogInstance().LogPrintf("[snapshot] validated snapshot (%.2f MB)\n",
        coins_cache.DynamicMemoryUsage() / (1000 * 1000));
    return true;
}
...
SnapshotCompletionResult ChainstateManager::MaybeCompleteSnapshotValidation()
{
    ...
    auto handle_invalid_snapshot = [&]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        ...
        LogInstance().LogError("[snapshot] !!! %s\n", user_error.original);
        LogInstance().LogError("[snapshot] deleting snapshot, reverting to validated chain, and stopping node\n");
    ...
    if (index_new.GetBlockHash() != snapshot_blockhash) {
        LogInstance().LogPrintf("[snapshot] supposed base block %s does not match the "
    ...
    if (!maybe_au_data) {
        LogInstance().LogPrintf("[snapshot] assumeutxo data not found for height "
    ...
    LogInstance().LogPrintf("[snapshot] computing UTXO stats for background chainstate to validate "
    ...
    if (!maybe_ibd_stats) {
        LogInstance().LogPrintf("[snapshot] failed to generate stats for validation coins db\n");
    ...
    if (AssumeutxoHash{ibd_stats.hashSerialized} != au_data.hash_serialized) {
        LogInstance().LogPrintf("[snapshot] hash mismatch: actual=%s, expected=%s\n",
    ...
    LogInstance().LogPrintf("[snapshot] snapshot beginning at %s has been fully validated\n",
...
void ChainstateManager::MaybeRebalanceCaches()
{
    ...
    if (ibd_usable && !snapshot_usable) {
        ...
        LogInstance().LogPrintf("[snapshot] allocating all cache to the snapshot chainstate\n");
...
bool ChainstateManager::DetectSnapshotChainstate()
{
    ...
    LogInstance().LogPrintf("[snapshot] detected active snapshot chainstate (%s) - loading\n",
...
Chainstate& ChainstateManager::ActivateExistingSnapshot(uint256 base_blockhash)
{
    ...
    LogInstance().LogPrintf("[snapshot] switching active chainstate to %s\n", m_snapshot_chainstate->ToString());
...
util::Result<void> Chainstate::InvalidateCoinsDBOnDisk()
{
    ...
    LogInstance().LogPrintf("[snapshot] renaming snapshot datadir %s to %s\n", dbpath, target);
    ...
    try {
        ...
        LogInstance().LogPrintf("%s: error renaming file '%s' -> '%s': %s\n",
...
bool ChainstateManager::DeleteSnapshotChainstate()
{
    ...
    if (!DeleteCoinsDBFromDisk(snapshot_datadir, /*is_snapshot=*/ true)) {
        LogInstance().LogPrintf("Deletion of %s failed. Please remove it manually to continue reindexing.\n",
                  fs::PathToString(snapshot_datadir));
        return false;
    }
    m_active_chainstate = m_ibd_chainstate.get();
...
bool ChainstateManager::ValidatedSnapshotCleanup()
{
    ...
    if (!ibd_chainstate_path_maybe || !snapshot_chainstate_path_maybe) {
        LogInstance().LogPrintf("[snapshot] snapshot chainstate cleanup cannot happen with "
    ...
    LogInstance().LogPrintf("[snapshot] deleting background chainstate directory (now unnecessary) (%s)\n",
    ...
    auto rename_failed_abort = [this](
        ...
        LogInstance().LogError("[snapshot] Error renaming path (%s) -> (%s): %s\n",
    ...
    LogInstance().LogPrintf("[snapshot] moving snapshot chainstate (%s) to "
    ...
    if (!DeleteCoinsDBFromDisk(tmp_old, /*is_snapshot=*/false)) {
        ...
        LogInstance().LogPrintf("Deletion of %s failed. Please remove it manually, as the "
                  "directory is now unnecessary.\n",
                  fs::PathToString(tmp_old));
    } else {
        LogInstance().LogPrintf("[snapshot] deleted background chainstate directory (%s)\n",
                  fs::PathToString(ibd_chainstate_path));
    }
    return true;
}
