...
MaybeEarlyExit<> Chainstate::MaybeUpdateMempoolForReorg(
...
MaybeEarlyExit<MempoolAcceptResult> AcceptToMemoryPool(Chainstate& active_chainstate, const CTransactionRef& tx,
...
MaybeEarlyExit<PackageMempoolAcceptResult> ProcessNewPackage(Chainstate& active_chainstate, CTxMemPool& pool,
...
MaybeEarlyExit<bool> AbortNode(BlockValidationState& state, const std::string& strMessage, const bilingual_str& userMessage)
{
    MAYBE_EXIT(AbortNode(strMessage, userMessage));
...
MaybeEarlyExit<bool> Chainstate::ConnectBlock(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex,
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlock(block, state, params.GetConsensus(), !fJustCheck, !fJustCheck)) {
        if (state.GetResult() == BlockValidationResult::BLOCK_MUTATED) {
            ...
            // problems.
            return BUBBLE_UP(AbortNode(state, "Corrupt block found indicating potential hardware failure; shutting down"));
        }
    ...
    EXIT_OR_IF_NOT(m_blockman.WriteUndoDataForBlock(blockundo, state, *pindex)) {
        return false;
    }
...
MaybeEarlyExit<bool> Chainstate::FlushStateToDisk(
    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            if (!CheckDiskSpace(gArgs.GetBlocksDirPath())) {
                return BUBBLE_UP(AbortNode(state, "Disk space is too low!", _("Disk space is too low!")));
            }
            {
            ...
            // Then update all block file information (which may refer to block and undo files).
            {
                ...
                if (!m_blockman.WriteBlockIndexDB()) {
                    return BUBBLE_UP(AbortNode(state, "Failed to write to block index database"));
                }
            }
        ...
        if (fDoFullFlush && !CoinsTip().GetBestBlock().IsNull()) {
            ...
            if (!CoinsTip().Flush())
                return BUBBLE_UP(AbortNode(state, "Failed to write to coin database"));
            m_last_flush = nNow;
    ...
    if (full_flush_completed) {
        ...
        return BUBBLE_UP(AbortNode(state, std::string("System error while flushing: ") + e.what()));
    }
    return true;
}

MaybeEarlyExit<> Chainstate::ForceFlushStateToDisk()
{
    BlockValidationState state;
...
MaybeEarlyExit<> Chainstate::PruneAndFlush()
{
    BlockValidationState state;
...
/** Disconnect m_chain's tip.
  ...
  * in any case).
  */
MaybeEarlyExit<bool> Chainstate::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    AssertLockHeld(cs_main);
 ...
 */
MaybeEarlyExit<bool> Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    AssertLockHeld(cs_main);
    if (m_mempool) AssertLockHeld(m_mempool->cs);
    ...
    if (!pblock) {
        ...
        if (!m_blockman.ReadBlockFromDisk(*pblockNew, *pindexNew)) {
            return BUBBLE_UP(AbortNode(state, "Failed to read block"));
        }
    ...
    {
        CCoinsViewCache view(&CoinsTip());
        EXIT_OR_DECL(bool rv, ConnectBlock(blockConnecting, state, pindexNew, view));
    ...
    if (this != &m_chainman.ActiveChainstate()) {
        ...
        MAYBE_EXIT(m_chainman.MaybeCompleteSnapshotValidation());
    }
 ...
 */
MaybeEarlyExit<bool> Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    AssertLockHeld(cs_main);
    if (m_mempool) AssertLockHeld(m_mempool->cs);
    ...
    while (m_chain.Tip() && m_chain.Tip() != pindexFork) {
        EXIT_OR_IF_NOT(DisconnectTip(state, &disconnectpool)) {
            ...
            MAYBE_EXIT(AbortNode(state, "Failed to disconnect block; see debug.log for details"));
            return false;
        }
...
MaybeEarlyExit<bool> Chainstate::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
...
MaybeEarlyExit<bool> Chainstate::PreciousBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    AssertLockNotHeld(m_chainstate_mutex);
    AssertLockNotHeld(::cs_main);
    {
    ...
    return BUBBLE_UP(ActivateBestChain(state, std::shared_ptr<const CBlock>()));
}

MaybeEarlyExit<bool> Chainstate::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    AssertLockNotHeld(m_chainstate_mutex);
    AssertLockNotHeld(::cs_main);
...
MaybeEarlyExit<bool> Chainstate::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    try {
        ...
        EXIT_OR_ASSIGN(blockPos , m_blockman.SaveBlockToDisk(block, pindex->nHeight, m_chain, dbp));
        if (blockPos.IsNull()) {
        ...
        return BUBBLE_UP(AbortNode(state, std::string("System error: ") + e.what()));
    }
...
MaybeEarlyExit<bool> ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
    AssertLockNotHeld(cs_main);

    {
        ...
        if (ret) {
            // Store to disk
            EXIT_OR_ASSIGN(ret , ActiveChainstate().AcceptBlock(block, state, &pindex, force_processing, nullptr, new_block, min_pow_checked));
        }
        if (!ret) {
...
MaybeEarlyExit<MempoolAcceptResult> ChainstateManager::ProcessTransaction(const CTransactionRef& tx, bool test_accept)
{
    AssertLockHeld(cs_main);
...
MaybeEarlyExit<bool> TestBlockValidity(BlockValidationState& state,
...
MaybeEarlyExit<> PruneBlockFilesManual(Chainstate& active_chainstate, int nManualPruneHeight)
{
    BlockValidationState state;
...
MaybeEarlyExit<> Chainstate::LoadMempool(const fs::path& load_path, FopenFn mockable_fopen_function)
{
    if (!m_mempool) return {};
    MAYBE_EXIT(::LoadMempool(*m_mempool, load_path, *this, mockable_fopen_function));
...
MaybeEarlyExit<VerifyDBResult> CVerifyDB::VerifyDB(
    Chainstate& chainstate,
...
MaybeEarlyExit<bool> ChainstateManager::LoadBlockIndex()
{
    ...
    if (!fReindex) {
        EXIT_OR_DECL(bool ret, m_blockman.LoadBlockIndexDB());
        if (!ret) return false;
...
MaybeEarlyExit<bool> Chainstate::LoadGenesisBlock()
{
    LOCK(cs_main);
    ...
    try {
        ...
        FlatFilePos blockPos;
        EXIT_OR_ASSIGN(blockPos , m_blockman.SaveBlockToDisk(block, 0, m_chain, nullptr));
        if (blockPos.IsNull()) {
...
MaybeEarlyExit<> Chainstate::LoadExternalBlockFile(
    FILE* fileIn,
    FlatFilePos* dbp,
    std::multimap<uint256, FlatFilePos>* blocks_with_unknown_parent)
{
    ...
    try {
            ...
            try {
...

                {
                    ...
                    if (!pindex || (pindex->nStatus & BLOCK_HAVE_DATA) == 0) {
                        ...
                        BlockValidationState state;
                        EXIT_OR_IF(AcceptBlock(pblock, state, nullptr, true, dbp, nullptr, true)) {
                            nLoaded++;
                        }
                        if (state.IsError()) {
                            break;
                ...
                if (m_blockman.IsPruneMode() && !fReindex && pblock) {
                    ...
                    BlockValidationState state;
                    EXIT_OR_IF_NOT(ActivateBestChain(state, pblock)) {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (m_blockman.ReadBlockFromDisk(*pblockrecursive, it->second)) {
                            ...
                            EXIT_OR_IF(AcceptBlock(pblockrecursive, dummy, nullptr, true, &it->second, nullptr, true)) {
                                nLoaded++;
        ...
        MAYBE_EXIT(AbortNode(std::string("System error: ") + e.what()));
    }
...
MaybeEarlyExit<bool> Chainstate::ResizeCoinsCaches(size_t coinstip_size, size_t coinsdb_size)
{
    AssertLockHeld(::cs_main);
        ...
        bool in_memory)
{
    ...
    EXIT_OR_DECL(bool snapshot_ok, this->PopulateAndValidateSnapshot(
    ...
    if (!snapshot_ok) {
        ...
        if (auto snapshot_datadir = node::FindSnapshotChainstateDir()) {
            ...
            if (!removed) {
                MAYBE_EXIT(AbortNode(strprintf("Failed to remove snapshot chainstate dir (%s). "
...
static MaybeEarlyExit<> SnapshotUTXOHashBreakpoint()
{
...
MaybeEarlyExit<SnapshotCompletionResult> ChainstateManager::MaybeCompleteSnapshotValidation(
      std::function<void(bilingual_str)> shutdown_fnc)
{
    ...
    auto handle_invalid_snapshot = [&]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) -> MaybeEarlyExit<> {
        ...
        MAYBE_EXIT(m_snapshot_chainstate->InvalidateCoinsDBOnDisk());
    ...
    if (index_new.GetBlockHash() != snapshot_blockhash) {
        ...
        MAYBE_EXIT(handle_invalid_snapshot());
    ...
    MAYBE_EXIT(m_ibd_chainstate->ForceFlushStateToDisk());
...
MaybeEarlyExit<> ChainstateManager::MaybeRebalanceCaches()
{
    AssertLockHeld(::cs_main);
...
MaybeEarlyExit<> Chainstate::InvalidateCoinsDBOnDisk()
{
    AssertLockHeld(::cs_main);
    ...
    try {
        ...
        MAYBE_EXIT(AbortNode(strprintf(
...
MaybeEarlyExit<bool> ChainstateManager::ValidatedSnapshotCleanup()
{
    AssertLockHeld(::cs_main);
    ...
    auto rename_failed_abort = [](
                                   fs::path p_old,
                                   fs::path p_new,
                                   const fs::filesystem_error& err) -> MaybeEarlyExit<> {
        ...
        MAYBE_EXIT(AbortNode(std::move(abort_string)));
            return {};
};
    ...
    try {
        ...
        MAYBE_EXIT(rename_failed_abort(ibd_chainstate_path, tmp_old, e));
        throw;
    }
    ...
    try {
        ...
        MAYBE_EXIT(rename_failed_abort(snapshot_chainstate_path, ibd_chainstate_path, e));
        throw;
    }
