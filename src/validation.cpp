...
MaybeEarlyExit<> CChainState::MaybeUpdateMempoolForReorg(
    ...
    bool fAddToMempool)
{
    ...
    while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
        ...
        if (!fAddToMempool || (*it)->IsCoinBase()) {
            ...
            if (atmp_ret.ShouldEarlyExit()) return BubbleUp(std::move(atmp_ret));
...
MaybeEarlyExit<MempoolAcceptResult> AcceptToMemoryPool(CChainState& active_chainstate, const CTransactionRef& tx,
...
MaybeEarlyExit<PackageMempoolAcceptResult> ProcessNewPackage(CChainState& active_chainstate, CTxMemPool& pool,
...
MaybeEarlyExit<bool> CChainState::ConnectBlock(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex,
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (fJustCheck)
        return true;
    auto write_ret = m_blockman.WriteUndoDataForBlock(blockundo, state, pindex, m_params);
...
MaybeEarlyExit<bool> CChainState::FlushStateToDisk(
...
{
    BlockValidationState state;
    auto flush_ret = this->FlushStateToDisk(state, FlushStateMode::ALWAYS);
    if (flush_ret.ShouldEarlyExit()) return BubbleUp(std::move(flush_ret));
    if (!*flush_ret){
...
MaybeEarlyExit<> CChainState::PruneAndFlush()
{
    BlockValidationState state;
    m_blockman.m_check_for_pruning = true;
    auto flush_ret = this->FlushStateToDisk(state, FlushStateMode::NONE);
    if (flush_ret.ShouldEarlyExit()) return BubbleUp(std::move(flush_ret));
    if (!*flush_ret) {
        LogPrintf("%s: failed to flush state (%s)\n", __func__, state.ToString());
    }
...
/** Disconnect m_chain's tip.
  ...
  * in any case).
  */
MaybeEarlyExit<bool> CChainState::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    AssertLockHeld(cs_main);
 ...
 */
MaybeEarlyExit<bool> CChainState::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    AssertLockHeld(cs_main);
    if (m_mempool) AssertLockHeld(m_mempool->cs);
    ...
    {
        CCoinsViewCache view(&CoinsTip());
        auto connect_ret = ConnectBlock(blockConnecting, state, pindexNew, view);
        if (connect_ret.ShouldEarlyExit()) return BubbleUp(std::move(connect_ret));
 ...
 */
MaybeEarlyExit<bool> CChainState::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    AssertLockHeld(cs_main);
    if (m_mempool) AssertLockHeld(m_mempool->cs);
    ...
    while (m_chain.Tip() && m_chain.Tip() != pindexFork) {
        ...
        if (disconnect_ret.ShouldEarlyExit()) return BubbleUp(std::move(disconnect_ret));
        if (!*disconnect_ret) {
...
MaybeEarlyExit<bool> CChainState::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
    ...
    do {
...

        {
            ...
            do {
                ...
                auto abc_ret = ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetHash() == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace);
    ...
    auto flush_ret = FlushStateToDisk(state, FlushStateMode::PERIODIC);
    if (flush_ret.ShouldEarlyExit()) return BubbleUp(std::move(flush_ret));
    if (!*flush_ret) {
        return false;
    }
...
MaybeEarlyExit<bool> CChainState::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    while (true) {
        ...
        MaybeUpdateMempoolForReorg(disconnectpool, /* fAddToMempool = */ (++disconnected <= 10) && *disconnect_ret);
        if (!*disconnect_ret) return false;
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    CBlockIndex* pindex{m_blockman.AddToBlockIndex(block, *this)};
...
MaybeEarlyExit<bool> CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    try {
        auto save_ret = m_blockman.SaveBlockToDisk(block, pindex->nHeight, m_chain, m_params, dbp);
        if (save_ret.ShouldEarlyExit()) return BubbleUp(std::move(save_ret));
        const auto& blockPos = *save_ret;
        if (blockPos.IsNull()) {
...
MaybeEarlyExit<bool> ChainstateManager::ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock>& block, bool force_processing, bool* new_block)
{
...

    {
        ...
        if (ret) {
            ...
            if (accept_ret.ShouldEarlyExit()) return BubbleUp(std::move(accept_ret));
            ret = *accept_ret;
        }
        if (!ret) {
    ...
    auto abc_ret = ActiveChainstate().ActivateBestChain(state, block);
    if (abc_ret.ShouldEarlyExit()) return BubbleUp(std::move(abc_ret));
    if (!*abc_ret) {
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    auto connect_ret = chainstate.ConnectBlock(block, state, &indexDummy, viewNew, true);
    if (connect_ret.ShouldEarlyExit()) return BubbleUp(std::move(connect_ret));
    if (!*connect_ret) {
        return false;
    }
...
{
    BlockValidationState state;
    auto flush_ret = active_chainstate.FlushStateToDisk(
...
MaybeEarlyExit<> CChainState::LoadMempool(bool persist)
{
    if (!m_mempool) return {};
    if (persist) {
...
MaybeEarlyExit<bool> CVerifyDB::VerifyDB(
    CChainState& chainstate,
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckLevel >= 4) {
        while (pindex != chainstate.m_chain.Tip()) {
            ...
            if (!ReadBlockFromDisk(block, pindex, consensus_params))
                return error("VerifyDB(): *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
            auto connect_ret = chainstate.ConnectBlock(block, state, pindex, coins);
            if (connect_ret.ShouldEarlyExit()) return BubbleUp(std::move(connect_ret));
            if (!*connect_ret) {
...
MaybeEarlyExit<bool> ChainstateManager::LoadBlockIndex()
{
    ...
    if (!fReindex) {
        auto load_ret = m_blockman.LoadBlockIndexDB(*this);
...
MaybeEarlyExit<bool> CChainState::LoadGenesisBlock()
{
    LOCK(cs_main);
    ...
    try {
        ...
        auto save_ret = m_blockman.SaveBlockToDisk(block, 0, m_chain, m_params, nullptr);
        if (save_ret.ShouldEarlyExit()) return BubbleUp(std::move(save_ret));
        const auto& blockPos = *save_ret;
        if (blockPos.IsNull()) {
            return error("%s: writing genesis block to disk failed", __func__);
        }
        CBlockIndex *pindex = m_blockman.AddToBlockIndex(block, m_chainman);
        ReceivedBlockTransactions(block, pindex, blockPos);
    } catch (const std::runtime_error& e) {
            ...
            try {
                ...
                uint256 hash = block.GetHash();
                {
                    ...
                    if (!pindex || (pindex->nStatus & BLOCK_HAVE_DATA) == 0) {
                      BlockValidationState state;
                      auto accept_ret = AcceptBlock(pblock, state, nullptr, true, dbp, nullptr);
                      if (accept_ret.ShouldEarlyExit()) return BubbleUp(std::move(accept_ret));
                      if (*accept_ret) { 
                          nLoaded++;
                      }
                ...
                if (hash == m_params.GetConsensus().hashGenesisBlock) {
                    BlockValidationState state;
                    auto abc_ret = ActivateBestChain(state, nullptr);
                    if (abc_ret.ShouldEarlyExit()) return BubbleUp(std::move(abc_ret));
                    if (!*abc_ret) {
                        break;
                    }
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, m_params.GetConsensus())) {
                            ...
                            auto accept_ret = AcceptBlock(pblockrecursive, dummy, nullptr, true, &it->second, nullptr);
                            if (accept_ret.ShouldEarlyExit()) return BubbleUp(std::move(accept_ret));
                            if (*accept_ret) {
                                nLoaded++;
...
MaybeEarlyExit<bool> CChainState::ResizeCoinsCaches(size_t coinstip_size, size_t coinsdb_size)
{
    ...
    try {
        ...
        while (num--) {
            ...
            if (nTime > nNow - nExpiryTimeout) {
                ...
                if (accepted.ShouldEarlyExit()) return BubbleUp(std::move(accepted));
        ...
        bool in_memory)
{
    ...
    auto snapshot_ok = this->PopulateAndValidateSnapshot(
        *snapshot_chainstate, coins_file, metadata);
    if (snapshot_ok.ShouldEarlyExit()) return BubbleUp(std::move(snapshot_ok));
...
std::optional<ChainstateLoadingError> ChainstateManager::LoadChainstate(bool fReset,
                                                     CTxMemPool* mempool,
                                                     bool fPruneMode,
                                                     const Consensus::Params& consensus_params,
                                                     ...
                                                     std::function<void()> coins_error_cb)
{
    ...
    LOCK(cs_main);
    InitializeChainstate(mempool);
    m_total_coinstip_cache = nCoinCacheUsage;
    m_total_coinsdb_cache = nCoinDBCache;
    ...
    if (!BlockIndex().empty() &&
            !m_blockman.LookupBlockIndex(consensus_params.hashGenesisBlock)) {
        return ChainstateLoadingError::ERROR_BAD_GENESIS_BLOCK;
    }

    // Check for changed -prune state.  What we are concerned about is a user who has pruned blocks
    // in the past, but is now trying to run unpruned.
    if (m_blockman.fHavePruned && !fPruneMode) {
    ...
    if (!fReindex) {
        auto load_ret = ActiveChainstate().LoadGenesisBlock();
        if (load_ret.ShouldEarlyExit()) return ChainstateLoadingError::ERROR_LOAD_GENESIS_BLOCK_FAILED;;
        if (!*load_ret) return ChainstateLoadingError::ERROR_LOAD_GENESIS_BLOCK_FAILED;
    }
...
std::optional<ChainstateLoadVerifyError> ChainstateManager::VerifyLoadedChainstate(bool fReset,
                                                                bool fReindexChainState,
                                                                const Consensus::Params& consensus_params,
