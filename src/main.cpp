...
void FinalizeNode(NodeId nodeid) {
    ...
    if (state->nMisbehavior == 0 && state->fCurrentlyConnected && g_connman) {
        g_connman->AddressCurrentlyConnected(state->address);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (send && g_connman && g_connman->OutboundTargetReached(true) && ( ((pindexBestHeader != NULL) && (pindexBestHeader->GetBlockTime() - mi->second->GetBlockTime() > nOneWeek)) || inv.type == MSG_FILTERED_BLOCK) && !pfrom->fWhitelisted)
                {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (!pfrom->fInbound)
        {
            ...
            if (pfrom->fOneShot || pfrom->nVersion >= CADDR_TIME_VERSION || (g_connman && g_connman->GetAddressCount() < 1000))
            {
                pfrom->PushMessage(NetMsgType::GETADDR);
                pfrom->fGetAddr = true;
            }
            if(g_connman)
                g_connman->MarkAddressGood(pfrom->addr);
        } else {
            if (g_connman && ((CNetAddr)pfrom->addr) == (CNetAddr)addrFrom)
            {
                g_connman->AddNewAddress(addrFrom, addrFrom);
                g_connman->MarkAddressGood(addrFrom);
            }
        }
    ...
    else if (strCommand == NetMsgType::ADDR)
    {
        ...
        if (pfrom->nVersion < CADDR_TIME_VERSION && g_connman && g_connman->GetAddressCount() > 1000)
            return true;
        if (vAddr.size() > 1000)
        {
        ...
        if(g_connman)
            g_connman->AddNewAddresses(vAddrOk, pfrom->addr, 2 * 60 * 60);
        if (vAddr.size() < 1000)
    ...
    {
        if (g_connman && g_connman->OutboundTargetReached(false) && !pfrom->fWhitelisted)
        {
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (state.fShouldBan) {
            if (pto->fWhitelisted)
                ...
                {
                    g_connman->Ban(pto->addr, BanReasonNodeMisbehaving);
                }
            }
        ...
        nNow = GetTimeMicros();
        if (pto->IsConnected() && state.nStallingSince && state.nStallingSince < nNow - 1000000 * BLOCK_STALLING_TIMEOUT) {
        ...
        if (pto->IsConnected() && state.vBlocksInFlight.size() > 0) {
        ...
        //
        vector<CInv> vGetData;
        if (pto->IsConnected() && !pto->fClient && (fFetch || !IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
        ...
        //
        while (pto->IsConnected() && !pto->mapAskFor.empty() && (*pto->mapAskFor.begin()).first <= nNow)
        {
