...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman)
{
    ...
    {
        // Must be happy with the version message before anything else
        LOCK(cs_main);
        Misbehaving(pfrom->GetId(), 1);
        return false;
    }
...
bool SendMessages(CNode* pto, CConnman& connman)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // If pto->fSuccessfullyConnected, the outgoing message serialization
        // version is set and can't change.
