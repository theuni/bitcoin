...
private:
    ...
    static void TimerCallback(evutil_socket_t fd, short flags, void *arg)
    {
        HTTPRPCTimer *self = static_cast<HTTPRPCTimer*>(arg);
        assert(self);
        self->func();
    }
public:
    HTTPRPCTimer(struct event_base *eventBase, boost::function<void(void)>& func, int64_t seconds):
        func(func)
    {
        ev = evtimer_new(eventBase, TimerCallback, this);
...
bool HTTPReq_JSONRPC(HTTPRequest *req)
{
    ...
    {
        req->WriteReply(HTTP_BADMETHOD, "JSONRPC server handles only POST requests");
        return false;
    }
    // Check authorization
