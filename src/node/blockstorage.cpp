...
MaybeEarlyExit<bool> BlockManager::LoadBlockIndex(const Consensus::Params& consensus_params)
{
    EXIT_OR_IF_NOT(m_block_tree_db->LoadBlockIndexGuts(consensus_params, [this](const uint256& hash) EXCLUSIVE_LOCKS_REQUIRED(cs_main) { return this->InsertBlockIndex(hash); })) {
        return false;
    }
...
MaybeEarlyExit<bool> BlockManager::LoadBlockIndexDB(const Consensus::Params& consensus_params)
{
    EXIT_OR_IF_NOT(LoadBlockIndex(consensus_params)) {
        return false;
    }
...
MaybeEarlyExit<> BlockManager::FlushUndoFile(int block_file, bool finalize)
{
    ...
    if (!UndoFileSeq().Flush(undo_pos_old, finalize)) {
        MAYBE_EXIT(AbortNode("Flushing undo file to disk failed. This is likely the result of an I/O error."));
    }
    return {};
}

MaybeEarlyExit<> BlockManager::FlushBlockFile(bool fFinalize, bool finalize_undo)
{
    LOCK(cs_LastBlockFile);
    FlatFilePos block_pos_old(m_last_blockfile, m_blockfile_info[m_last_blockfile].nSize);
    if (!BlockFileSeq().Flush(block_pos_old, fFinalize)) {
        MAYBE_EXIT(AbortNode("Flushing block file to disk failed. This is likely the result of an I/O error."));
    }
...
MaybeEarlyExit<bool> BlockManager::FindBlockPos(FlatFilePos& pos, unsigned int nAddSize, unsigned int nHeight, CChain& active_chain, uint64_t nTime, bool fKnown)
{
    LOCK(cs_LastBlockFile);
    ...
    if (!fKnown) {
        ...
        if (out_of_space) {
            return BUBBLE_UP(AbortNode("Disk space is too low!", _("Disk space is too low!")));
        }
...
MaybeEarlyExit<bool> BlockManager::WriteUndoDataForBlock(const CBlockUndo& blockundo, BlockValidationState& state, CBlockIndex* pindex, const CChainParams& chainparams)
{
    AssertLockHeld(::cs_main);
    ...
    if (pindex->GetUndoPos().IsNull()) {
        ...
        if (!UndoWriteToDisk(blockundo, _pos, pindex->pprev->GetBlockHash(), chainparams.MessageStart())) {
            return BUBBLE_UP(AbortNode(state, "Failed to write undo data"));
        }
...
MaybeEarlyExit<FlatFilePos> BlockManager::SaveBlockToDisk(const CBlock& block, int nHeight, CChain& active_chain, const CChainParams& chainparams, const FlatFilePos* dbp)
{
    ...
    if (dbp == nullptr) {
        if (!WriteBlockToDisk(block, blockPos, chainparams.MessageStart())) {
            MAYBE_EXIT(AbortNode("Failed to write block"));
            return FlatFilePos();
        }
    }
    return blockPos;
...
MaybeEarlyExit<> ThreadImport(ChainstateManager& chainman, std::vector<fs::path> vImportFiles, const ArgsManager& args, const fs::path& mempool_path)
{
...

    {
        ...
        if (fReindex) {
            ...
            while (true) {
                ...
                MAYBE_EXIT(chainman.ActiveChainstate().LoadExternalBlockFile(file, &pos, &blocks_with_unknown_parent));
    ...
    MAYBE_EXIT(chainman.ActiveChainstate().LoadMempool(mempool_path));
    return {};
}
} // namespace node
