...
MiniMiner::MiniMiner(const CTxMemPool& mempool, const std::vector<COutPoint>& outpoints)
{
    ...
    for (const auto& outpoint : outpoints) {
        ...
        if (const auto ptx{mempool.GetConflictTx(outpoint)}) {
            ...
            for (const auto& desc_txiter : descendants) {
                m_to_be_replaced.insert(desc_txiter->first.GetTx().GetHash());
            }
        }
    }
    ...
    for (const auto& txiter : cluster) {
        if (!m_to_be_replaced.count(txiter->first.GetTx().GetHash())) {
            auto [mapiter, success] = m_entries_by_txid.emplace(txiter->first.GetTx().GetHash(), MiniMinerMempoolEntry(txiter));
            m_entries.push_back(mapiter);
        } else {
            auto outpoints_it = m_requested_outpoints_by_txid.find(txiter->first.GetTx().GetHash());
    ...
    for (const auto& txiter : cluster) {
        const auto& txid = txiter->first.GetTx().GetHash();
        ...
        for (const auto& desc_txiter : descendants) {
            const auto txid_desc = desc_txiter->first.GetTx().GetHash();
