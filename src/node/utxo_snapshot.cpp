...
bool WriteSnapshotBaseBlockhash(Chainstate& snapshot_chainstate)
{
    ...
    AutoFile afile{file};
    if (afile.IsNull()) {
        LogInstance().LogPrintf("[snapshot] failed to open base blockhash file for writing: %s\n",
    ...
    if (afile.fclose() != 0) {
        LogInstance().LogPrintf("[snapshot] failed to close base blockhash file %s after writing\n",
                  fs::PathToString(write_to));
        return false;
    }
    return true;
...
std::optional<uint256> ReadSnapshotBaseBlockhash(fs::path chaindir)
{
    if (!fs::exists(chaindir)) {
        LogInstance().LogPrintf("[snapshot] cannot read base blockhash: no chainstate dir "
    ...
    if (!fs::exists(read_from)) {
        LogInstance().LogPrintf("[snapshot] snapshot chainstate dir is malformed! no base blockhash file "
    ...
    AutoFile afile{file};
    if (afile.IsNull()) {
        LogInstance().LogPrintf("[snapshot] failed to open base blockhash file for reading: %s\n",
            read_from_str);
        return std::nullopt;
    }
    ...
    if (std::fgetc(afile.Get()) != EOF) {
        LogInstance().LogPrintf("[snapshot] warning: unexpected trailing data in %s\n", read_from_str);
    } else if (std::ferror(afile.Get())) {
        LogInstance().LogPrintf("[snapshot] warning: i/o error reading %s\n", read_from_str);
    }
