    ...
    const ChainstateLoadOptions& options) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    pblocktree = std::make_unique<BlockTreeDB>(DBParams{
        ...
        .options = chainman.m_options.block_tree_db}, LogInstance());
    ...
    for (Chainstate* chainstate : chainman.GetAll()) {
        LogInstance().LogPrintf("Initializing chainstate %s\n", chainstate->ToString());
                                    ...
                                    const ChainstateLoadOptions& options)
{
    if (!chainman.AssumedValidBlock().IsNull()) {
        LogInstance().LogPrintf("Assuming ancestors of block %s have valid signatures.\n", chainman.AssumedValidBlock().GetHex());
    } else {
        LogInstance().LogPrintf("Validating signatures for all blocks.\n");
    }
    LogInstance().LogPrintf("Setting nMinimumChainWork=%s\n", chainman.MinimumChainWork().GetHex());
    if (chainman.MinimumChainWork() < UintToArith256(chainman.GetConsensus().nMinimumChainWork)) {
        LogInstance().LogPrintf("Warning: nMinimumChainWork set below default value of %s\n", chainman.GetConsensus().nMinimumChainWork.GetHex());
    }
    if (chainman.m_blockman.GetPruneTarget() == BlockManager::PRUNE_TARGET_MANUAL) {
        LogInstance().LogPrintf("Block pruning enabled.  Use RPC call pruneblockchain(height) to manually prune block and undo files.\n");
    } else if (chainman.m_blockman.GetPruneTarget()) {
        LogInstance().LogPrintf("Prune configured to target %u MiB on disk for block and undo files.\n", chainman.m_blockman.GetPruneTarget() / 1024 / 1024);
    }
    ...
    if (has_snapshot && options.wipe_chainstate_db) {
        LogInstance().LogPrintf("[snapshot] deleting snapshot chainstate due to reindexing\n");
    ...
    if (snapshot_completion == SnapshotCompletionResult::SKIPPED) {
        ...
        LogInstance().LogPrintf("[snapshot] cleaning up unneeded background chainstate, then reinitializing\n");
