    ...
    const ChainstateLoadOptions& options) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (!chainman.BlockIndex().empty() &&
        ...
        return ChainstateLoadResult{ChainstateLoadStatus::FAILURE_INCOMPATIBLE_DB, _("Incorrect or no genesis block found. Wrong datadir for network?")};
    }

    // Check for changed -prune state.  What we are concerned about is a user who has pruned blocks
    // in the past, but is now trying to run unpruned.
    if (chainman.m_blockman.m_have_pruned && !options.prune) {
        return ChainstateLoadResult{ChainstateLoadStatus::FAILURE, _("You need to rebuild the database using -reindex to go back to unpruned mode.  This will redownload the entire blockchain")};
    }
    ...
    for (Chainstate* chainstate : chainman.GetAll()) {
        ...
        if (chainstate->CoinsDB().NeedsUpgrade()) {
            return ChainstateLoadResult{ChainstateLoadStatus::FAILURE_INCOMPATIBLE_DB, _("Unsupported chainstate database format found. "
        ...
        if (!chainstate->ReplayBlocks()) {
            return ChainstateLoadResult{ChainstateLoadStatus::FAILURE, _("Unable to replay blocks. You will need to rebuild the database using -reindex-chainstate.")};
        }
    ...
    if (!options.reindex) {
        ...
        if (std::any_of(chainstates.begin(), chainstates.end(),
            ...
            return ChainstateLoadResult{ChainstateLoadStatus::FAILURE, strprintf(_("Witness data for blocks after height %d requires validation. Please restart with -reindex."),
    ...
    for (Chainstate* chainstate : chainman.GetAll()) {
        if (!is_coinsview_empty(chainstate)) {
            ...
            if (tip && tip->nTime > GetTime() + MAX_FUTURE_BLOCK_TIME) {
                return ChainstateLoadResult{ChainstateLoadStatus::FAILURE, _("The block database contains a block which appears to be from the future. "
            ...
            case VerifyDBResult::INTERRUPTED:
                return ChainstateLoadResult{ChainstateLoadStatus::INTERRUPTED, _("Block verification was interrupted")};
            case VerifyDBResult::CORRUPTED_BLOCK_DB:
                return ChainstateLoadResult{ChainstateLoadStatus::FAILURE, _("Corrupted block database detected")};
            case VerifyDBResult::SKIPPED_L3_CHECKS:
                if (options.require_full_verification) {
                    return ChainstateLoadResult{ChainstateLoadStatus::FAILURE_INSUFFICIENT_DBCACHE, _("Insufficient dbcache for block verification")};
                }
                break;
